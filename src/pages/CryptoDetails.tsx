...
const CryptoDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCryptoData = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get<CryptoData>(`https://api.coingecko.com/api/v3/coins/${id}`, {
          params: {
            localization: false,
            tickers: false,
            market_data: true,
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      ...
      try {
        const response = await axios.get<MarketChartData>(`https://api.coingecko.com/api/v3/coins/${id}/market_chart`, {
          params: {
            vs_currency: 'usd',
            days: '30',
          },
                ...
                <TabsContent value="overview" className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-xl font-bold">${cryptoData.market_data.current_price.usd.toLocaleString()}</div>
                    </div>
                    <div>
                      <div className="text-sm text-muted-foreground">Volume 24h</div>
                      <div className="text-xl font-bold">${cryptoData.market_data.total_volume.usd.toLocaleString()}</div>
                    </div>
                    <div>
                      <div className="text-sm text-muted-foreground">Capitalisation Marché</div>
                      <div className="text-xl font-bold">${cryptoData.market_data.market_cap.usd.toLocaleString()}</div>
                    </div>
                    <div>
                      <div className="text-sm text-muted-foreground">Changement 24h</div>
                      <div className={`text-xl font-bold ${cryptoData.market_data.price_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">${cryptoData.market_data.market_cap.usd.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Volume total</span>
              <span className="font-medium">${cryptoData.market_data.total_volume.usd.toLocaleString()}</span>
            </div>
