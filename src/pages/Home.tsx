  ...
  useEffect(() => {
    const smoothFollow = () => {
      setAnimatedPosition((prev) => ({
        x: prev.x + (mousePosition.x - prev.x) * 0.2, // Interpolation pour un mouvement fluide
        y: prev.y + (mousePosition.y - prev.y) * 0.2,
      }));
    };

    const animationFrame = requestAnimationFrame(smoothFollow); // Utilise requestAnimationFrame pour réduire le lag
  ...
  return (
      <div className="flex flex-col min-h-screen">
        <div
            ...
            style={{
              transform: `translate(${animatedPosition.x - 24}px, ${animatedPosition.y - 80}px)`, // Ajuste la hauteur (-28 au lieu de -24)
              transition: 'transform 0.05s linear', // Adoucit les mouvements
            }}
        ></div>
